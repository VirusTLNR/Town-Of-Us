...
namespace TownOfUs
{
    ...
    {
        public static List<System.Tuple<string, string>> list = new List<System.Tuple<string, string>>()
        {
            //add a new line here for each version so can make sure the updating does not go accross versions.
            System.Tuple.Create("2.7.1","2021-06-30")
        };
    ...
    public class AutoUpdater
    {
        ...
        {
            public static string updateCheckURI = "https://api.github.com/repos/Eisbison/TheOtherRoles/releases/latest";
            ...
            public static async Task<bool> checkVersions(System.Version latestVersion)
            {

                string currentVersGameVers = null;
                string latestVersGameVers = null;
                bool curFlag = false;
                bool latFlag = false;

                foreach (var version in Versions.list)
                {
                    if(version.Item1==latestVersion.ToString())
                    {
                        latestVersGameVers = version.Item2;
                        latFlag = true;
                    }
                    if(version.Item1== TownOfUs.GetVersion().ToString())
                    {
                        currentVersGameVers = version.Item2;
                        curFlag = true;
                    }
                }
                return true;
            ...
            public static async Task<bool> checkForUpdate()
            {
                try
                {
                    ...
                    var response = await http.GetAsync(new System.Uri(updateCheckURI), HttpCompletionOption.ResponseContentRead);
                    ...
                    int diff = TownOfUs.GetVersion().CompareTo(ver);
                    if (diff < 0)
                    { // Update required
                        hasUpdate = true;
                ...
                {
                    Reactor.PluginSingleton<TownOfUs>.Instance.Log.LogError(ex.ToString());
